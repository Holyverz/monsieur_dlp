...
async fn main() {
    ...
    let lines = youtube::filesystem::read_songs(common::constants::youtube_songs_file()).expect("failed to read songs");
    ...
    // Wait for all tasks to complete
     ...
     match youtube::filesystem::add_success_downloads(&success, common::constants::youtube_songs_historic_path()) {
        Ok(()) => print!("✅ Files saved!"),
        Err(e) => eprintln!("Files writting failed unexpectedly: {}", e)
     }
    ...
    // Mount device
    match ios::mounting::mount_app(common::constants::APP_BUNDLE_ID, &PathBuf::from(common::constants::mounting_path())).await {
